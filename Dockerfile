...
# this size peaks near ~4.1 GB and OOMs a build worker at 4 GB. 6144 provides 2 GB headroom.
...
RUN --mount=type=cache,id=s/92ca8a61-c1ba-421f-a389-d48ac7258c2d-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && OMNIROUTE_BUILD_MEMORY_MB=6144 NODE_OPTIONS="--max-old-space-size=6144" npm run build \
