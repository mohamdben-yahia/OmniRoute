...
# -*- coding: utf-8 -*-
"""
Test Windsurf Claude Opus 4.5, 4.6, 4.7 models specifically.
Tests the latest Claude Opus versions to see if they're available.
"""
import sys
import os
import time
...
print('='*70)
print('WINDSURF CLAUDE OPUS 4.5, 4.6, 4.7 TEST')
print('='*70)
print()
...
print()
print('[3/3] Starting Claude Opus 4.5, 4.6, 4.7 test...')
print()

# Claude Opus 4.5, 4.6, 4.7 variants to test
OPUS_4567_MODELS = [
    # Claude Opus 4.7 variants
    ('claude-opus-4.7', 'Claude Opus 4.7', 'Anthropic'),
    ('claude-opus-4-7', 'Claude Opus 4.7 (alt)', 'Anthropic'),
    ('claude-4.7-opus', 'Claude 4.7 Opus', 'Anthropic'),

    # Claude Opus 4.6 variants
    ('claude-opus-4.6', 'Claude Opus 4.6', 'Anthropic'),
    ('claude-opus-4-6', 'Claude Opus 4.6 (alt)', 'Anthropic'),
    ('claude-4.6-opus', 'Claude 4.6 Opus', 'Anthropic'),

    # Claude Opus 4.5 variants
    ('claude-opus-4.5', 'Claude Opus 4.5', 'Anthropic'),
    ('claude-opus-4-5', 'Claude Opus 4.5 (alt)', 'Anthropic'),
    ('claude-4.5-opus', 'Claude 4.5 Opus', 'Anthropic'),

    # Additional possible naming patterns
    ('claude-opus-4.7-20250514', 'Claude Opus 4.7 (2025-05-14)', 'Anthropic'),
    ('claude-opus-4.6-20250514', 'Claude Opus 4.6 (2025-05-14)', 'Anthropic'),
    ('claude-opus-4.5-20250514', 'Claude Opus 4.5 (2025-05-14)', 'Anthropic'),
]

TEST_PROMPT = "You are Claude Opus. What is your exact model version? Answer in one short sentence with your model name and version number."

print('='*70)
print('TEST DES MODÈLES CLAUDE OPUS 4.5, 4.6, 4.7')
print('='*70)
print(f'\nTimestamp: {datetime.now().isoformat()}')
print(f'Modèles à tester: {len(OPUS_4567_MODELS)}')
print(f'Méthode: Test direct sans AssignModel\n')

results = []

for idx, (model_id, model_name, provider) in enumerate(OPUS_4567_MODELS, 1):
    print(f'\n{"="*70}')
    print(f'[{idx}/{len(OPUS_4567_MODELS)}] Test: {model_name}')
    print(f'{"="*70}')
    ...
    try:
        ...
        if send_result.get('status') != 200:
            ...
            # Check if it's a 500 error (likely BYOK required)
            if send_result.get('status') == 500:
                result['byok_required'] = True
                result['error'] = 'BYOK configuration required'

            print(f'  ✗ SendMessage failed: {send_result.get("status")}')
            results.append(result)
            continue
        ...
        claude_patterns = [
            (rb'claude[- ]?opus[- ]?4\.7', 'Claude Opus 4.7'),
            (rb'claude[- ]?opus[- ]?4\.6', 'Claude Opus 4.6'),
            (rb'claude[- ]?opus[- ]?4\.5', 'Claude Opus 4.5'),
            (rb'claude[- ]?opus[- ]?4', 'Claude Opus 4'),
            (rb'claude[- ]?4\.7', 'Claude 4.7'),
            (rb'claude[- ]?4\.6', 'Claude 4.6'),
            (rb'claude[- ]?4\.5', 'Claude 4.5'),
    ...
    # Pause between tests
    if idx < len(OPUS_4567_MODELS):
        print(f'\n  Pausing 2s...')
        time.sleep(2)

# Generate report
print('\n\n' + '='*70)
print('RAPPORT FINAL - CLAUDE OPUS 4.5, 4.6, 4.7')
print('='*70)
...
byok_required = [r for r in results if r.get('byok_required', False)]
failed = [r for r in results if r['status'] == 'failed' and not r.get('byok_required', False)]
...
if byok_required:
    ...
    for r in byok_required:
        print(f'  • {r["model_name"]} - Configuration clé API requise')
...
# Save results
output_file = 'windsurf_opus_4567_test.json'
