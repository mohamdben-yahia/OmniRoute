...
let lastRedisErrorSignature: string | null = null;
...
function normalizeRedisErrorMessage(error: unknown): string {
  if (error instanceof Error) {
    ...
    return parts.join(": ") || "Unknown Redis error";
  }

  if (typeof error === "string") {
    return error.trim() || "Unknown Redis error";
  }

  try {
    const serialized = JSON.stringify(error);
    return serialized && serialized !== "{}" ? serialized : "Unknown Redis error";
  } catch {
    return "Unknown Redis error";
  }
}

function shouldLogRedisError(signature: string, now = Date.now()): boolean {
  if (
    signature === lastRedisErrorSignature &&
    now - lastRedisErrorLoggedAt < REDIS_ERROR_LOG_WINDOW_MS
  ) {
    return false;
  }
...
export function __resetRedisLoggingStateForTests() {
  lastRedisErrorSignature = null;
  lastRedisErrorLoggedAt = 0;
}

export function __formatRedisErrorForTests(error: unknown): string {
  return normalizeRedisErrorMessage(error);
}

export function __shouldLogRedisErrorForTests(signature: string, now?: number): boolean {
  return shouldLogRedisError(signature, now);
}
...
export function getRedisClient() {
  if (!redisClient) {
    ...
    redisClient.on("error", (err) => {
      const message = normalizeRedisErrorMessage(err);
